...
<body>
    ...
    else if($_SESSION['userlevel'] == 3)
    {
        ...
        $userid = $_SESSION['userid'];
        echo ' <form method="POST" action="view_report.php">
        <input class="back_btn" type="submit" name="returnback" value="Back"/>
        </form>';
    }

    echo "<h2 style='text-align:center'>Sorted Report (sort by Room Number)</h2>"; 
    $formData = "SELECT * FROM applicationForm WHERE verifyStatus='Approve' OR verifyStatus='Reject' ORDER BY roomNo";
